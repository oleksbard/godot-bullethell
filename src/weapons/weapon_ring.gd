...
var _sfx: Node
var _impact: Node
var _sfx_by_key: Dictionary = {}     # per-weapon ShotSfx, keyed by clip set
...
func _assign_targets() -> void:
	...
	for i in _guns.size():
		...
		if _mounts[i] != null and player.has_method("get_hand_target"):
			...
			t = live[i % live.size()]             # floating: i-th closest; extra guns reuse existing imps so none idle
		var rng_max: float = _guns[i].def.range if _guns[i].def != null else MAX_RANGE
		if is_instance_valid(t) and origin.distance_to(t.global_position) <= rng_max:
			_guns[i].set_target(t)
		else:
			_guns[i].clear_target()           # out of this weapon's range -> holds fire
...
## A ShotSfx configured for `def`'s clip set (cached per clip set). Falls back to the
## shared pistol _sfx when there's no def or it has no clips of its own.
func _sfx_for(def: Object) -> Node:
	if def == null or def.shot_clips.is_empty():
		return _sfx
	var key := str(def.shot_clips)
	if not _sfx_by_key.has(key):
		var s: Node = ShotSfxScript.new()
		add_child(s)
		s.configure(def.shot_clips, def.shot_peaks_db)
		_sfx_by_key[key] = s
	return _sfx_by_key[key]
...
func _on_gun_fired(origin: Vector3, target: Node3D, damage: float, aim_dir: Vector3, gun: Node3D) -> void:
	...
	p.damage = damage
	p.aim_dir = aim_dir
	if gun.def != null:                  # per-weapon bolt: speed, blood spray + range from the def
		p.speed = gun.def.proj_speed
		p.blood_min = gun.def.blood_min
